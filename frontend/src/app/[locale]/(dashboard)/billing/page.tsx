...
export default function BillingPage() {
    ...
    if (loading) {
        return <div className="p-8 text-white">{t("loadingBilling")}</div>;
    }

    if (!subscription) {
        return <div className="p-8 text-red-400">{t("failedToLoad")}</div>;
    }
        ...
        <div className="container max-w-5xl py-10 space-y-10">
            ...
            <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                        <div className="space-y-2">
                            <div className="flex justify-between text-sm">
                                <span className="text-white/70">{t("diagramsUsed")}</span>
                                <span className="text-white font-mono">{subscription.diagrams_used} / {subscription.diagrams_limit}</span>
                            </div>
                            <Progress value={diagramsPercent} className="h-2" />
                        </div>
                        <div className="space-y-2">
                             <div className="flex justify-between text-sm">
                                <span className="text-white/70">{t("aiRequests")}</span>
                ...
                <GlassCard className="flex flex-col justify-center items-center text-center space-y-4 bg-gradient-to-br from-white/5 to-purple-500/5">
                     <h2 className="text-2xl font-bold text-white">{t("needMorePower")}</h2>
            ...
            <h2 className="text-2xl font-bold text-white">{t("availablePlans")}</h2>
            <div className="grid md:grid-cols-3 gap-6">
                 <GlassCard className="border-t-4 border-t-gray-500 flex flex-col gap-4">
                     <div>
                        <h3 className="text-xl font-bold">{t("free")}</h3>
                        <div className="text-3xl font-bold mt-2">$0<span className="text-sm font-normal text-white/50">{t("perMonth")}</span></div>
                     </div>
                     <ul className="space-y-2 text-sm text-white/70 flex-1">
                        <li className="flex gap-2"><CheckCircle className="w-4 h-4 text-green-400"/> {t("features.diagrams5")}</li>
                        <li className="flex gap-2"><CheckCircle className="w-4 h-4 text-green-400"/> {t("features.basicAI")}</li>
                     </ul>
                     <Button variant="outline" disabled className="w-full border-white/10 text-white/50">{t("currentPlan")}</Button>
                 </GlassCard>
                 ...
                 <GlassCard className="border-t-4 border-t-purple-500 bg-purple-500/5 flex flex-col gap-4 relative">
                     ...
                     <div>
                        ...
                        <div className="text-3xl font-bold mt-2">$29<span className="text-sm font-normal text-white/50">{t("perMonth")}</span></div>
                     </div>
                     <ul className="space-y-2 text-sm text-white/70 flex-1">
                 ...
                 <GlassCard className="border-t-4 border-t-blue-500 flex flex-col gap-4">
                     ...
                     <Button variant="outline" className="w-full border-white/20 hover:bg-white/10" onClick={() => handleUpgrade('enterprise')}>{t("contactSales")}</Button>
                 </GlassCard>
            </div>
        </div>
    );
